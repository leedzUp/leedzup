...
class ProductCore extends ObjectModel
{
    ...
    {
        $cacheKey = 'Product::getImages_'.(int)$this->id.'_'.(int)$id_lang;
        if (!Cache::isStored($cacheKey)) {
            ...
            $sql->leftJoin(
                'image_lang', 
                'il', 
                'i.`id_image` = il.`id_image` AND il.`id_lang` = '.(int)$id_lang
            );
            $sql->where('i.`id_product` = '.(int)$this->id);
            //$sql->orderBy('i.`position`');
