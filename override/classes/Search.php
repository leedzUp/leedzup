    ...
    */
    public static function find(
        ...
        if (!empty($encodedFacets)) {
            ...
            // Récupération des ID des caractéristiques
            $id_feature_ville = self::getFeatureIdByName('feat_ville', $id_lang);
            $id_feature_departement = self::getFeatureIdByName('feat_departement', $id_lang);
            $id_feature_zip_code = self::getFeatureIdByName('feat_zip_code', $id_lang);
            $id_feature_pays = self::getFeatureIdByName('feat_pays', $id_lang);
            $id_feature_region = self::getFeatureIdByName('feat_region', $id_lang);
            $id_feature_region_touristique = self::getFeatureIdByName('feat_region_touristique', $id_lang);
            $id_feature_type_du_bien = self::getFeatureIdByName('feat_type_du_bien', $id_lang);
                ...
                foreach ($decodedFacets as $facet => $values) {
                    if (!empty($values)) {
                        if ($facet === 'max_price') {
                            ...
                            // Recherche sur plusieurs caractéristiques liées à la localisation
                            $facetConditions[] = "p.id_product IN (
                                ...
                                WHERE fp.id_feature IN ($id_feature_ville, $id_feature_departement, $id_feature_zip_code, $id_feature_pays, $id_feature_region, $id_feature_region_touristique)
                                AND fvl.value LIKE '%" . pSQL($values) . "%'
                                AND fvl.id_lang = " . (int)Context::getContext()->language->id . "
                            )";
                        } elseif ($facet === 'property_type') {
                            // Recherche sur la caractéristique "Type de bien"
                            $facetConditions[] = "p.id_product IN (
                                SELECT fp.id_product 
                                FROM " . _DB_PREFIX_ . "feature_product fp
                                WHERE fp.id_feature = $id_feature_type_du_bien
                                AND fp.id_feature_value = (
                                    SELECT fvl.id_feature_value 
                                    FROM " . _DB_PREFIX_ . "feature_value_lang fvl 
                                    WHERE fvl.value LIKE '%" . pSQL($values) . "%' 
                                    AND fvl.id_lang = " . (int)Context::getContext()->language->id . "
                                    LIMIT 1
                                )
                            )";
    ...
    private static function getFeatureIdByName($featureName, $id_lang)
    {
        ...
        return null; // Retourne null si la caractéristique n'existe pas
    }
